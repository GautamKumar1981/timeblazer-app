...
const Check: React.FC<{ text: string; ok?: boolean }> = ({ text, ok = true }) => (
  <div style={{ display: 'flex', gap: 10, padding: '6px 0', fontSize: 13, color: ok ? '#d1d5db' : '#6b7280' }}>
    <span style={{ color: ok ? '#22c55e' : '#4b5563', fontWeight: 700 }}>{ok ? '✓' : '·'}</span>
    {text}
  </div>
);
  ...
  return pageLayout(
    <>
            ...
            {sub.is_trial_active && (
              <>
                ...
                <div style={{ fontSize: 13, color: '#9ca3af' }}>
                  You have <strong style={{ color: '#e5e7eb' }}>{sub.trial_days_remaining} day{sub.trial_days_remaining !== 1 ? 's' : ''}</strong> remaining
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20, marginBottom: 32 }}>
        {/* Free plan */}
        <div style={{
          backgroundColor: '#16152e', borderRadius: 14, padding: '26px 24px',
          border: '1px solid rgba(255,255,255,0.08)',
        }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#6b7280', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>Free</div>
          <div style={{ fontSize: 32, fontWeight: 900, color: '#e5e7eb', marginBottom: 4 }}>£0</div>
          <div style={{ fontSize: 12, color: '#4b5563', marginBottom: 20 }}>Always free, no card needed</div>
          {FEATURES_FREE.map((f) => <Check key={f} text={f} ok />)}
          {FEATURES_PREMIUM.slice(0, 3).map((f) => <Check key={f} text={f} ok={false} />)}
          <div style={{ fontSize: 11, color: '#4b5563', marginTop: 6 }}>+ more premium features…</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: '#c4b5fd', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>
            ✨ Premium
          </div>
          <div style={{ fontSize: 32, fontWeight: 900, color: '#e9d5ff', marginBottom: 4 }}>
            £2.99<span style={{ fontSize: 14, fontWeight: 400, color: '#9ca3af' }}>/month</span>
          </div>
          <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>
            Billed monthly · Cancel anytime
          </div>
          <div style={{ fontSize: 11, color: '#22c55e', marginBottom: 20, fontWeight: 600 }}>
      ...
      <div style={{
        backgroundColor: '#16152e', borderRadius: 12, padding: '22px 24px',
        border: '1px solid rgba(139,92,246,0.2)', marginBottom: 24,
      }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: '#c4b5fd', marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
          {[
            ...
            <div key={title} style={{ backgroundColor: '#1a1830', borderRadius: 10, padding: '14px 16px' }}>
              <div style={{ fontSize: 22, marginBottom: 8 }}>{icon}</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#e5e7eb', marginBottom: 6 }}>{title}</div>
      ...
      <div style={{
        ...
        {[
          ...
          <div key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '14px 0' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#e5e7eb', marginBottom: 6 }}>{q}</div>
